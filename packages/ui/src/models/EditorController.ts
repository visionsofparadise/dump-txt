...
import { snapshotView, type SessionState, type TextMatch, type TextRange, type ViewSnapshot } from "./SessionState";
...
export class EditorController {
	...
	readonly #appearance = new Compartment();
	...
	attach(parent: HTMLElement): void {
		...
		if (typeof ResizeObserver !== "undefined") {
			...
			this.#resizeObserver = new ResizeObserver(() => {
				if (width === parent.clientWidth && height === parent.clientHeight) return;
	...
	select(ranges: ReadonlyArray<TextRange>, mainIndex = 0): void {
		const view = this.#view;

		if (!view) throw new Error("The editor surface must be mounted before selecting text.");
