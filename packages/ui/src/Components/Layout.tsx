...
export const Layout = scope(({ ref, surface, context: dumpContext }: LayoutProps) => {
	...
	const context = useMemo<ChromeContext>(() => ({ ...dumpContext, chrome, surface }), [chrome, dumpContext, surface]);
	...
	useEffect(() => {
		if (surface.current) surface.current.dataset.theme = theme;

		void context.main.setTheme(theme).catch((error: unknown) => console.error(error));
	}, [context.main, surface, theme]);
